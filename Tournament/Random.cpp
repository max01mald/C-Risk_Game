	...
	if (p->getCountries().size() > 0)
	{
		...
		while (break1 != -1)
		{
			...
			while (break2 != -1)
			{
				...
				choice = random(0, counter-2);
				
				cout << "Random player has selected " << choice+1 << endl;
			...
			int initialUnits = p->getCountries()[choice].getNumUnits();
			...
			p->getCountries()[choice].setNumUnits(initialUnits + choice2); //adds the armies to the previous number of armies on that country
			cout << "final num units " << p->getCountries()[choice].getNumUnits() << endl;
			p->setPlayerUnits(armies);


			cout << "\n" << choice2 << " armies have been placed on " << p->getCountries()[choice].getname() << endl;
			cout << "Now there are " << p->getCountries()[choice].getNumUnits() << " armies on " << p->getCountries()[choice].getname() << " .\n" << endl;
			...
			while(answer != "no")
			{
				...
				
				{
					...
					
					{
						...
						{
							answer = "no";
							cout << "End of Attack phase Random.\n" << endl;
						}
						else
						{
							answer = "yes";
						...
						answer = "no";
						cout << "No countries to attack. End of Attack phase Random.\n" << endl;
					}
				}
				else {
					answer = "no";
					cout << "No countries to attack from. End of Attack phase Random.\n" << endl;
				}
			}// end of while loop	
		}
	}

Country& Random::playerCountrySelection(playerClass* actor, game* g, Country &stop)
{
	...
	for(int i=0; i<actor->getCountries().size(); i++)
	{
		if(arr3[choice].getname() == actor->getCountries()[i].getname())
		{
			temp2 = i;
		}
...
Country& Random::chooseDefendingCountry(playerClass* actor, game* g, Country* player, Country &stop)
{
	...
	for(int i = 0; i < v2.size() && i < v1.size(); i++)
	{
		...
		if(v1[i] > v2[i])
		{
			...
			if(player2->getNumUnits() == 0)
			{
				...
				if (actor->getHaveDrawn() == false) {
					g->getDeck()->draw(actor->getPlayerHand());
					actor->setHaveDrawn(true);
					cout << "The player has drawn a card\n";
					cout << "The player has a total of " << actor->getPlayerHand()->getSize() << " card(s)." << endl;
				}
				...
				{
					int army = player1->getNumUnits() - 1;//maximum possible number of armies to move
	...
	if (counter == 1) {
		...
		if (p->getCountries()[i].getname() == name)
		{
			...
			for (int j = 0; j < p->getCountries()[i].getN_vec().size(); j++)
			{
				...
				{
					if (p->getCountries()[i].getN_vec()[j] == p->getCountries()[k].getname())
					{
			...
			h = (1, p->getCountries()[temp2].getNumUnits() - 1);
