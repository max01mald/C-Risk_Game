	...
	for(int i = 0; i < v2.size() && i < v1.size() && i < 2; i++)
	{
		...
		if(v1[i] > v2[i])
		{
			...
			if(player2->getNumUnits() == 0)
			{
				...
				cout << "The new owner is " << g->getPlayer(position1).getCountries()[position2].getowner()<<endl;
...
void Human::rollDicesDefendingPlayer(playerClass* villan, Country* cntry)
{
	...
	{
		cout << "From 1-2 , enter amount of dice rolls to put: " << endl;
...
void Human::fortify(playerClass* p, game* g) 
{
	...
	while (break1 != -1)
	{
		...
		for (int i = 0; i<p->getCountries().size(); i++)
		{
			if (p->getCountries()[i].getNumUnits() > 1)
			{
				...
				arr3.push_back(p->getCountries()[i]);
	...
	if (counter == 1)
	{
			cout << "No Country has a friendly neighbour. " << endl;
			if (p->getCountries().size() != 0)
			{
				for (int l = 0; l < 100; l++)
				{
				...
				delete[]arr2;
				arr2 = NULL;
				cout << "Fortification phase ended Human.\n" << endl;
				return;
			}
	}
	...
	for (int i = 0; i< p->getPlayerCountriesSize(); i++)
	{
		...
		if (p->getCountries()[i].getname() == name)
		{
			...
			while (break1 != -1)
			{
				...
				cout << p->getCountries()[temp].getname() << " has " << p->getCountries()[temp2].getNumUnits()-1 << " units to move." << endl;
