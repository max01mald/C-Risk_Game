...
filler::filler(): stream_name("No one")
{
}
...
void filler::is_file()
{
	while(!inStream.eof())
	{
		...
		{
			std::cerr << "Error this is not a valid map, [Map]" << std::endl;
			exit(1);
		}	
	}
}
...
vector<Continent> filler::Cont_fill(vector<Country> cntry, std::string S_name)
{
	int cc = 0;
	store = "";
	line = 'a';
	...
	{
		std::cerr << "Error this is not a valid map[store_vec_size]" << std::endl;
			exit(1);
	}
...
vector<Country> filler::Countr_fill(std::string S_name)
{
	...
	if (counter == 0)
	{
		std::cerr << "Error this is not a valid map[not filled]" << std::endl;
		exit(1);
	}
...
void filler::open_map(std::string map_name)
{
	std::cout << "The map file has been opened" << std::endl;
