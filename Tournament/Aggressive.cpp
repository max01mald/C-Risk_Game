	...
	if(actor->getCountries().size() > 0)
	{
		...
		while(break1 != -1)
		{
			...
			if (player1->getname() != "") {
				...
				{
					break1 = -1;
					cout << "End of Aggressive Attack" << endl;
					break;
				}
				...
				cout << "End of Attack phase Aggressive.\n" << endl;
				break1 = -1;
			}
		}// end of while loop
	}
...
Country& Aggressive::playerCountrySelection(playerClass* actor, game* g, Country &stop)
{
	...
	if (max == 1) {
		cout << "No countries with more than 1 unit." << endl;
		return stop;
	}
...
Country& Aggressive::chooseDefendingCountry(playerClass* actor, game* g, Country* player, Country &stop)
{
	int choice =0;
	int choice2 =0;
	...
	if(counter == 0)
	{	
		cout<<"No Countris to attack."<<endl;
		return stop;
	} 
...
void Aggressive::compareDiceRolls(playerClass* actor, game* g, playerClass& villan, Country* player1, Country* player2)
{
	...
	for (int i = 0; i < actor->getPlayerDiceFactory().getNumDiceRolls(); i++)
	{
		for (int j = i + 1; j < actor->getPlayerDiceFactory().getNumDiceRolls(); j++)
		{
			if (actor->getPlayerDiceFactory().getV()[i] < actor->getPlayerDiceFactory().getV()[j])
			{
				temp1 = actor->getPlayerDiceFactory().getV()[j];
				actor->getPlayerDiceFactory().getV()[j] = actor->getPlayerDiceFactory().getV()[i];
				actor->getPlayerDiceFactory().getV()[i] = temp1;
			}
		}
	}
	//sort array from largest to smallest
	for (int i = 0; i < villan.getPlayerDiceFactory().getNumDiceRolls(); i++)
	{
		for (int j = i + 1; j < villan.getPlayerDiceFactory().getNumDiceRolls(); j++)
		{
			...
			{
				temp2 = villan.getPlayerDiceFactory().getV()[j];
				villan.getPlayerDiceFactory().getV()[j] = villan.getPlayerDiceFactory().getV()[i];
				villan.getPlayerDiceFactory().getV()[i] = temp2;
			}
		}
	}
	...
	for(int i = 0; i < v2.size() && i < v1.size() && i < 2; i++)
	{
...
void Aggressive::rollDicesDefendingPlayer(playerClass* villan, Country* cntry)
{
	...
	if(cntry->getNumUnits() > 2)
	{
		temp = 2;
	}
	else if(cntry->getNumUnits() <= 2)
	{
		temp = 1;
	}
	cout <<"Defending Player picked " << temp << " Dice to roll"<< endl;
...
void Aggressive::fortify(playerClass* p, game* g)
{
	...
	if(p->getCountries().size() > 0)
	{
		...
		cout << "Selecting neighbour country of destination.\n" << p->getCountries()[choice2].getname() << endl;
