...
void game::AllocateCountries(Map& map)
{
	...
	while (this->getPlayer(this->getCastSize()-1).getPlayerUnits() > 0)
	{
		...
		while (test == false)
		{
			...
			{
				if (this->getPlayer(playCounter).getCountries()[i].getname() == this->getPlayer(playCounter).getCountries()[i].getname()) //stringInput
				{
...
void game::attack(playerClass& actor)
	...
	Country& game::playerCountrySelection(playerClass& actor)
	{
		...
		while(actor.getCountries()[choice-1].getNumUnits() < 2 || choice > actor.getPlayerCountriesSize())
		{
		...
		return actor.getCountries()[choice-1];
	}
