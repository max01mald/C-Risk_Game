...
void Benevolent::fortify(playerClass* p, game* g) 
{
	...
	if(p->getCountries().size() > 0)
	{
		...
		cout << "Selecting weakest neighbour country of " << p->getCountries()[choice].getname() << endl;
		...
		for (int j=0; j< p->getCountries()[choice].getN_vec().size(); j++)
		{
			...
			{
				if(p->getCountries()[choice].getN_vec()[j] == p->getCountries()[k].getname())
				{			
		...
		cout << "Benevolent Fortification phase ended. \n" << endl;
