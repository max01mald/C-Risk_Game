...
Country::Country(): name(""), owner(""), continent(""), cord1(0), cord2(0),
numUnits(0), numOrder(0)
{
}
...
Country::Country(string new_name, string new_owner, int units)
{
	name = new_name;
	owner = new_owner;
	numUnits = units;
...
Country::Country(string new_name, string new_owner, int units, vector<Country> nCount)
{
	name = new_name;
	owner = new_owner;
	numUnits = units;
	neighboorCountries = nCount;

}

Country::Country(Country* cntry)
{
	name = cntry->name;
	owner = cntry->owner;
	...
	cord1 = cntry->cord1;
	cord2 = cntry->cord2;
	numUnits = cntry->numUnits;
	numOrder = cntry->numOrder;
	
	neigh_vec = cntry->neigh_vec;
	
	num_of_neigh = neigh_vec.size();
	
	for (int i=0; i<num_of_neigh; i++)
	{
		Country* country = new Country("empty");
		country = cntry->getN_Ptr(i);
		count_ptr_vec.push_back(country);
...
vector<string> Country::getN_vec()
{
	return neigh_vec;
}
...
int& Country::getNumUnits()
{
	return numUnits;
}

int& Country::getNumOrder()
{
	return numOrder;
}
...
void Country::setNumUnits(int units)
{
	numUnits = units;
}

void Country::setNumOrder(int order)
{
	numOrder = order;
}

void Country::setNeighboorCountries(vector<Country> nCount)
{
			neighboorCountries = nCount;
}
...
void Country::setN(int pos, Country& country)
{
	count_ptr_vec[pos] = &country;
